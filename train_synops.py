...
- An additional term to reduce the number of synaptic operations is added to the loss function.
    ...
    while True:
        for inputs in dataloader:
            ...
            if data.seq_num >= len(data.files):
                ...
                mlflow.log_metric("synops", synops, step=data.epoch)
            ...
            assert model.encoder_unet.encoders[0].recurrent_block.batch_size == config["loader"]["batch_size"], 'Wrong batch size in encoders'
            assert model.encoder_unet.pooling.batch_size == config["loader"]["batch_size"], 'Wrong batch size in pooling'
            ...
            if loss_function.num_passes >= config["data"]["passes_loss"]:
                ...
                # synops loss
                synops += calculate_synops(model)

                # loss
                loss = loss_function() 
                train_loss += loss.item() #+ synops_loss.item()
                bw_loss += loss #+ synops_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        default="configs/ann_conv_c1.yml",
